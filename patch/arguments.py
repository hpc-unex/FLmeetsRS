...
def add_args():
    ...
    # default arguments
    parser.add_argument("--algorithm", type=str, default="x")
    ...
    #argparser.add_argument("--dataset", type=str, default="UCMERCED", help="which dataset")
    #argparser.add_argument("--tr_percent", type=float, default=0.2)
    #argparser.add_argument("--modelname", type=str, default="r18")  
    #argparser.add_argument("--epochs", type=int, default=100)
    #argparser.add_argument("--batch_size", type=int, default=100)
    #argparser.add_argument("--lr", type=float, default=0.001)
    #argparser.add_argument("--part_file", type=str, default="./partfile.part")
    #argparser.add_argument("--balanced", type=int, default=1)
    #argparser.add_argument("--speeds", type=str, default=".")  
    #argparser.add_argument("--dir", type=str, default="./outputs/")  
    #argparser.add_argument("--iter", type=int, default=1)
    #argparser.add_argument("--with_test", type=bool, default=True)
    #argparser.add_argument("--mode", type=str, default="baseline")
    # remote sensing parameters
    #print(args.tr_percent); exit()
...
class Arguments:
    ...
    def __init__(self, config, cmd_args, training_type=None, comm_backend=None, override_cmd_args=True):
        # set the command line arguments
        cmd_args_dict = cmd_args.__dict__
        for arg_key, arg_val in cmd_args_dict.items():
            setattr(self, arg_key, arg_val)

        self.get_default_yaml_config(config, cmd_args, training_type, comm_backend)
    ...
    def get_default_yaml_config(self, config, cmd_args, training_type=None, comm_backend=None):
        if cmd_args.yaml_config_file == "" and config == "":
        ...
        #print(config); exit()
        if config != "":
           cmd_args.yaml_config_file = config
...
def load_arguments(config, algorithm=None, training_type=None, comm_backend=None):
    cmd_args = add_args()

    # Load all arguments from YAML config file
    args = Arguments(config, cmd_args, training_type, comm_backend)
