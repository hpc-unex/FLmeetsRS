...
def _data_transforms_RSICB256():
    RSICB256_MEAN = [0.4243, 0.4323, 0.3928] #[0.485, 0.456, 0.406]
    RSICB256_STD = [0.1307, 0.1252, 0.1205] #[0.229, 0.224, 0.225]
...
def load_RSICB256_data(datadir, args):
    train_transform, test_transform = _data_transforms_RSICB256()

    RSICB256_train_ds = RSICB256_truncated(
        datadir, train=True, download=True, transform=train_transform, args=args
    )
    RSICB256_test_ds = RSICB256_truncated(
        datadir, train=False, download=True, transform=test_transform, args=args
    )

    X_train, y_train = RSICB256_train_ds.data, RSICB256_train_ds.target
    X_test, y_test = RSICB256_test_ds.data, RSICB256_test_ds.target

    return (X_train, y_train, X_test, y_test)

def partition_data(dataset, datadir, partition, n_nets, alpha, args):
    logging.info("*********partition data***************")
    X_train, y_train, X_test, y_test = load_RSICB256_data(datadir, args)
    #print(X_train); exit()
    ...
    if partition == "homo":
        ...
        dataidx_map_file_path = (
            "./data_preprocessing/non-iid-distribution/RSICB256/net_dataidx_map.txt"
        )
        net_dataidx_map = read_net_dataidx_map(dataidx_map_file_path)

    if partition == "hetero-fix":
        distribution_file_path = (
            "./data_preprocessing/non-iid-distribution/RSICB256/distribution.txt"
        )
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, args=None):
    return get_dataloader_RSICB256(datadir, train_bs, test_bs, dataidxs, args)
...
def get_dataloader_RSICB256(datadir, train_bs, test_bs, dataidxs=None, args=None):
    dl_obj = RSICB256_truncated

    transform_train, transform_test = _data_transforms_RSICB256()

    train_ds = dl_obj(
        datadir, dataidxs=dataidxs, train=True, transform=transform_train, download=True, args=args)
    test_ds = dl_obj(datadir, train=False, transform=transform_test, download=True, args=args)
...
def load_partition_data_distributed_RSICB256(
    process_id,
    dataset,
    data_dir,
    partition_method,
...
def load_partition_data_RSICB256(
    dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, args
):
    (
        X_train,
        y_train,
        ...
        dataset, data_dir, partition_method, client_number, partition_alpha, args
    )
